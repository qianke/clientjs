(function(win, doc){
	...
	var NSPClient = function(appid,option){
		...
		this.sid = option && option.sid ? this.sid = option.sid : '';
		this.secret = option && option.secret ? this.secret = option.secret : '';
		this.debug = option && option.debug ? this.debug = option.debug : false;
	}
	/**
     * 检查是否登录
	...
	NSPClient.prototype.api = function(name,param,callback){
			...
			API["nsp_svc"] = name;
			self.send(self.url,self.sid,self.secret,API,success);
	}
	/**
	 ...
	 * @return undefined
	 */
	NSPClient.prototype.send = function(url,sid,secret,data,callback){
			var self = this;
			data["nsp_ts"] = new Date().getTime();
			data["nsp_sid"] = sid;
			data["nsp_fmt"] = 'JS';
			data['nsp_cb'] = '_jqjsp';
			data = NSPHelper.encrypt(secret,data);
			ajax({
				url: url,
				data:data,
				dataType: "jsonp",
